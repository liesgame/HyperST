...
from pretrained.scgpt_spatial import logger
...
class Preprocessor:
    ...
    def __init__(
        ...
        add_cell_norm (:class:`bool`, default: ``False``):
            Whether to perform additional cell-wise normalization by dividing each cell's expression by the dataset's mean.
        add_gene_norm (:class:`str`, optional):
            The path to a CSV file containing pre-computed population-level gene statistics to perform gene-wise normalization.
        vocab (:class:`callable`, optional):
            A function or mapping to convert gene names to integer indices. This is required when using `add_gene_norm`.
        result_add_norm_key (:class:`str`, default: ``"X_add_normed"``):
            The key of :class:`~anndata.AnnData` to store the additional normalized data (either cell-wise or gene-wise normalization).
    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None) -> Dict:
        ...
        # Additional Norm: Cell level divide by dataset mean
        ...
        # Additional Norm: Gene level divide by pre-computed population-level gene mean
